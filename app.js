...
OBR.onReady(async () => {
    try {
        ...
        function renderScoreboard(gameData) {
            ...
            if (players.length === 0) {
                scoreboardList.innerHTML = "<p>You are the first one here!</p>";
            }
        ...
        if (initialGameData[myId] && initialGameData[myId].points !== undefined) {
            myPoints = initialGameData[myId].points;
            pointsDisplay.value = myPoints;
            maxBidDisplay.innerText = Math.max(0, myPoints);
        }
        
        // Force an initial render so it doesn't stay stuck on "Loading players..."
        renderScoreboard(initialGameData);
        ...
        console.error("Bid Tracker Error:", error);
        document.getElementById("room-status-text").innerText = "Error loading. Check browser console.";
    }
});
